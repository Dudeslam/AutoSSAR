...
void getWithinRangeCallback(const std_msgs::Int8MultiArray::ConstPtr& array)
{

    int i = 0;
	// print all the remaining numbers
	for(std::vector<int_fast8_t>::const_iterator it = array->data.begin(); it != array->data.end(); ++it)
	{
		UAVsInRange[i] = *it;
		i++;

        switch(UAVsInRange[i]){
            case 0:
                inRangeUAV0 = true;
                break;
            case 1:
...
int main (int argc, char* argv[]){
    ...
    ros::Subscriber map_global_uav1 = nh.subscribe(otherUAV0+"/map_Global", 10, getGlobalMapCallback);
    ros::Subscriber map_global_uav2 = nh.subscribe(otherUAV1+"/map_Global", 10, getGlobalMapCallback);

    ros::Subscriber within_range_uav1 = nh.subscribe(selfUAV+"/within_range_array", 10, getWithinRangeCallback);
    ros::Publisher map_pub = nh.advertise<sensor_msgs::PointCloud2>(selfUAV+"/map_Global", 1000);
