...
//for coord.
const int BsplineOptimizer::COORDINATION   = (1 << 5);
...
//for coord.
const int BsplineOptimizer::GUIDE_COORD_PHASE = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::GUIDE | BsplineOptimizer::COORDINATION;
const int BsplineOptimizer::NORMAL_COORD_PHASE =
    BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::DISTANCE | BsplineOptimizer::FEASIBILITY | BsplineOptimizer::COORDINATION;
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  if (cost_function_ & COORDINATION) cost_str += " coord ";
...
void BsplineOptimizer::setCoordPath(const vector<Eigen::Vector3d>& coord_pt){
  coord_pt_ = coord_pt;
}
...
void BsplineOptimizer::calcCoordCost(const vector<Eigen::Vector3d>& q, double& cost,
                                     vector<Eigen::Vector3d>& gradient) {
  cost = 0.0;
  Eigen::Vector3d zero(0, 0, 0);
  std::fill(gradient.begin(), gradient.end(), zero);

  float safe_dist = 2.0; //1.5m safe distance
  int end_idx = q.size() - order_;
  int coord_idx = coord_pt_.size();

  for (int i = order_; i < min(end_idx,coord_idx); i++) {

    float d2d_dist = (q[i] - coord_pt_[i]).norm();
    if(d2d_dist > safe_dist){
      cost += 0;
      //do nothing
    }
    else{
      cost += 0.7*pow((d2d_dist-safe_dist),2);
      //push to the opposit direction
      gradient[i] += (coord_pt_[i] - q[i]);
    }
  }

  //cout << "[DEBUG] BsplineOptimizer::calcCoordCost: "<< cost <<" \n";
}
...
void BsplineOptimizer::combineCost(const std::vector<double>& x, std::vector<double>& grad,
  ...
  double f_smoothness, f_distance, f_feasibility, f_endpoint, f_guide, f_waypoints, f_coord;
  f_smoothness = f_distance = f_feasibility = f_endpoint = f_guide = f_waypoints = f_coord = 0.0;
  ...
  if (cost_function_ & COORDINATION) {
    // ROS_INFO("[DEBUG]: cost_function_ & COORDINATION");
    calcCoordCost(g_q_, f_coord, g_coord_);
    f_combine += lambda5_ * f_coord;
    for (int i = 0; i < variable_num_ / dim_; i++)
      for (int j = 0; j < dim_; j++) grad[dim_ * i + j] += lambda5_ * g_coord_[i + order_](j);
  }
