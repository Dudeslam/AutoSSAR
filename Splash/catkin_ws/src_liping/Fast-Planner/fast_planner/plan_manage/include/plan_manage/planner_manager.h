...
public:
  ...
  bool topoReplan(bool collide, bool coord_enable); 
  ...
  void setCoordWaypoints(vector<Eigen::Vector3d>& coord_waypoints);
...
private:
  ...
  void findCollisionRange2(vector<Eigen::Vector3d>& colli_start, vector<Eigen::Vector3d>& colli_end,
                          vector<Eigen::Vector3d>& start_pts, vector<Eigen::Vector3d>& end_pts);

  void optimizeTopoBspline(double start_t, double duration, vector<Eigen::Vector3d> guide_path,
                           int traj_id);
  void optimizeTopoBsplineCoord(double start_t, double duration, vector<Eigen::Vector3d> guide_path,
  ...
  void refineTraj(NonUniformBspline& best_traj, double& time_inc, bool coord_req);
