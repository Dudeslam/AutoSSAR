...
void FastPlannerManager::setCoordWaypoints(vector<Eigen::Vector3d>& coord_waypoints){
  plan_data_.coord_waypoints_ = coord_waypoints;
}
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {  // simply truncate the segment and do nothing
    ...
    cout << "[DEBUG] topoReplan() no collide \n";

  } else {
    cout << "[DEBUG] topoReplan() replan+optimize \n";
...
bool FastPlannerManager::topoReplan(bool collide, bool coord_enable) {
  ros::Time t1, t2;
  ...
  if ((!collide)&&(!coord_enable)) {  // simply truncate the segment and do nothing
    refineTraj(init_traj, time_inc);
    local_data_.position_traj_ = init_traj;
    global_data_.setLocalTraj(init_traj, t_now, local_traj_duration + time_inc + t_now, time_inc);
    cout << "[DEBUG] topoReplan() no collide \n";
    ...
    if(collide){
      ...
      findCollisionRange2(colli_start, colli_end, start_pts, end_pts);
    }
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      ...
      vector<thread> optimize_threads;
      for (int i = 0; i < select_paths.size(); ++i) {
          optimize_threads.emplace_back(&FastPlannerManager::optimizeTopoBsplineCoord, this, t_now,
                              local_traj_duration, select_paths[i], i, coord_enable);
          // optimize_threads.emplace_back(&FastPlannerManager::optimizeTopoBspline, this, t_now,
          //                     local_traj_duration, select_paths[i], i);
          // optimizeTopoBspline(t_now, local_traj_duration,
          // select_paths[i], origin_len, i);
      ...
      selectBestTraj(best_traj);
      //refineTraj(best_traj, time_inc);
      refineTraj(best_traj, time_inc, coord_enable);
...
//extend the function for coordination
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc, bool coord_req) {
  ros::Time t1 = ros::Time::now();
  time_inc     = 0.0;
  double    dt, t_inc;
  const int max_iter = 1;
  ...
  int cost_function;
  if(coord_req){
    cost_function = BsplineOptimizer::NORMAL_COORD_PHASE;
  }else{
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  bspline_optimizers_[traj_id]->setCoordPath(plan_data_.coord_waypoints_);
...
//for the case of coordination required
void FastPlannerManager::optimizeTopoBsplineCoord(double start_t, double duration, vector<Eigen::Vector3d> guide_path,
                          int traj_id, bool coord_req){
  ...
  // for coordination , coordination specific PHASE code will be used
  Eigen::MatrixXd opt_ctrl_pts1;
  if(coord_req){
    opt_ctrl_pts1 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
        ctrl_pts, dt, BsplineOptimizer::GUIDE_COORD_PHASE, 0, 1);
  }else{
    opt_ctrl_pts1 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
        ctrl_pts, dt, BsplineOptimizer::GUIDE_PHASE, 0, 1);
  }
  ...
  if(coord_req){
    opt_ctrl_pts2 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
        opt_ctrl_pts1, dt, BsplineOptimizer::NORMAL_COORD_PHASE, 1, 1);
  }else{
    opt_ctrl_pts2 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
        opt_ctrl_pts1, dt, BsplineOptimizer::NORMAL_PHASE, 1, 1);
  }
...
void FastPlannerManager::findCollisionRange(vector<Eigen::Vector3d>& colli_start,
  ...
  //find the section of the path that is occupied by the obstacles.
  double t_s = -1.0, t_e;
...
void FastPlannerManager::findCollisionRange2(vector<Eigen::Vector3d>& colli_start,
                                            vector<Eigen::Vector3d>& colli_end,
                                            vector<Eigen::Vector3d>& start_pts,
                                            vector<Eigen::Vector3d>& end_pts) {
  bool               last_safe = true, safe;
  ...
  for (double tc = t_m; tc <= t_mp + 1e-4; tc += 0.05) {
    Eigen::Vector3d ptc = initial_traj->evaluateDeBoor(tc);
    //simulate a vitual collision
    safe = true;
    if((tc>t_mp-1.0)&&(tc<t_mp-0.5)){
      safe = false;
    }
